...
def update_hand(hand, word):
    ...
    """
    dir = {}
    for key, value in hand.iteritems():
        dir[key] = value
    for i in word:
        dir[i] = dir.get(i, 0) - 1
    return dir
...
def play_hand(hand, word_list):
    ...
    * The final score is displayed.
    ...
    while True:
        # show hand
        v = []
        vs = list(hand.iteritems())
        vs.sort(key = lambda i: i[1])
        for k, value in vs:
            for i in range(value):
                v.append(k)
    
        print "Current hand:", " ".join(v)

        # get input
        word = raw_input("Enter word, or a . to indicate that you are finished: ")
        if word == '.': break
        # check hand
        h = update_hand(hand, word)
        if not all(map(lambda v: v >= 0, h.values())):
        ...
        if all(map(lambda v: v == 0, hand.values())):
            break
